...
class survey():   
    def __init__(self, username, password, address, databasename):
        ...
        self.refresh_from_database('bot_survey_questions')
    def refresh_from_database(self,table_name):
        try:
    ...
    def get_next_question_id(self, question_id, answer):
        question_df = self.refresh_from_database("logic_branches") #logic_branches is the table name saving the logic branches.
        sub_df = question_df.loc[(question_df['question_id'] == question_id) 
                                 & ((question_df['test_text'].isnull()) | (question_df['test_text'].apply(lambda x: str(x).lower() in answer.lower())))]
        
        return sub_df['next_question_id'][0] if sub_df['next_question_id'].shape[0] == 1 else print('error')
